...
import utils.pb.payment.payment_pb2 as payment
import utils.pb.payment.payment_pb2_grpc as payment_grpc
...
class ExecutorService(executor_grpc.ExecutorServiceServicer):
    ...
    def start_election(self):
        ...
        for node_id in higher_nodes:
            try:
                # Assuming executors are available on internal network names like executor_1:{EXECUTOR_PORT}
    ...
    def check_leader_health(self):
        ...
        try:
            with grpc.insecure_channel(f'executor_{self.leader_id}:{EXECUTOR_PORT}') as channel:
                stub = executor_grpc.ExecutorServiceStub(channel)
